...
class Prop(db.Model):
    ...
    total_contracts = db.Column(db.Integer, default=100)  # Total contracts available
    sold_contracts = db.Column(db.Integer, default=0)  # Contracts sold so far
    available_contracts = db.Column(db.Integer, default=100)  # Available for trading
    ...
    def can_buy_contracts(self, quantity=1):
        ...
        if self.game_status == GameStatus.UPCOMING:
            # Can buy up to total_contracts
            return self.available_contracts >= quantity
        elif self.game_status == GameStatus.LIVE:
            # Can only buy previously sold contracts
            return self.sold_contracts >= quantity and self.available_contracts >= quantity
        
        return False
    
    def can_sell_contracts(self, user_id, quantity=1):
        """Check if contracts can be sold"""
        if not self.can_trade():
            return False
