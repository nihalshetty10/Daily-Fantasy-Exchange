...
if 'props' in data:
    ...
    for prop_id, prop_data in list(data['props'].items())[:10]:
        position = prop_data['player_info']['position']
        if position == 'P':
            pitcher_count += 1
            print(f"Pitcher found: {prop_data['player_info']['name']} - {prop_data['player_info']['team_name']}")
    
    print(f"Total pitchers in first 10 props: {pitcher_count}") 