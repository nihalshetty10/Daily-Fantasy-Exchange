"""
NBA LSTM Model for Player Prop Predictions
...
class NBAModel:
    def __init__(self):
        ...
        self.models = {}
        self.prop_types = ['POINTS', 'REBOUNDS', 'ASSISTS', 'STEALS', 'BLOCKS']
        
    def scrape_player_stats(self, player_name, days_back=30):
        """Scrape player's recent performance data from Basketball-Reference"""
        try:
            ...
            player_url = f"https://www.basketball-reference.com{player_link['href']}"
            
            # Get player's game log
            game_log_url = player_url.replace('.html', '/gamelog/')
                ...
                for row in rows[:days_back]:  # Last 30 games
                    ...
                    if len(cells) > 15:
                        game_stats = {
                            ...
                            'points': float(cells[8].text.strip() or 0),
                            'rebounds': float(cells[12].text.strip() or 0),
                            'assists': float(cells[13].text.strip() or 0),
                            'steals': float(cells[14].text.strip() or 0),
                            'blocks': float(cells[15].text.strip() or 0)
                        }
